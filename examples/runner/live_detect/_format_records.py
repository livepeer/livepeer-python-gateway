...
def main() -> None:
    for line in sys.stdin:
        ...
        kind = rec.get("type")
        if kind == "transcript":
            print("[T{:3d}] {}".format(rec["index"], rec["text"]), flush=True)
        elif kind == "detection":
            objs = ", ".join(
                "{}({:.2f})".format(o["label"], o["conf"]) for o in rec["objects"]
            )
            print("[D{:3d}] frame={} {}".format(rec["index"], rec["frame"], objs), flush=True)
