...
class LivePaymentSession:
    ...
    async def send_payment(
        ...
        *,
        ...
        session-scoped ``{control_url}/payment`` which 404s once the session is
        released. Without it the payment goes to the orchestrator's generic
        ``/payment`` endpoint, which credits the payer balance blindly.
